 ...
 *
 * ESTRATEGIA ANTI-OOM:
 * - Baixa o ZIP em streaming (arrayBuffer necessario para o parser local).
 * - Decomprime o CSV e percorre byte-a-byte em linha-a-linha SEM materializar
...
async function scanInfDiarioForCnpjs(
  ...
  const csvBytes = await extractFirstCsvFromZip(zipBytes);
    ...
    // Processamos em latin-1 (ISO-8859-1): cada byte e um char code <= 255,
...
async function fetchCvmFundName(cnpjDigits: string): Promise<string | null> {
  ...
  try {
    // 1) Tentativa rapida (CSV direto) costuma ser mais leve que o ZIP
...
export async function handleGetQuotes(req: Request, env: WorkerEnv): Promise<Response> {
  runtimeEnv = env;
